...
function Resolve-Antigravity {
    # Prefer a previously configured path when it still exists.
    if ($env:MULTIGRAVITY_APP -and
        (Test-Path $env:MULTIGRAVITY_APP -PathType Leaf)) {
        return $env:MULTIGRAVITY_APP
    }

    # Discover the installation when the configured path is missing or stale.
    $app = Find-Antigravity

    if ($app) {
        try {
            # Persist the discovered executable path for future sessions.
            ...
            # Make it available immediately to the current PowerShell process.
            $env:MULTIGRAVITY_APP = $app
        } catch {
            Write-Warning "Could not persist MULTIGRAVITY_APP: $($_.Exception.Message)"
        }
...
function Invoke-LaunchProfile {
    ...
    if ($ArgsToForward) {
        Start-Process -FilePath $APP -WorkingDirectory (Split-Path $APP -Parent) -ArgumentList $ArgsToForward
    }
    else {
        Start-Process -FilePath $APP -WorkingDirectory (Split-Path $APP -Parent) -WorkingDirectory (Split-Path $APP -Parent)
    }
}
